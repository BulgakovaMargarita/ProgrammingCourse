...
\begin{enumerate}
\item \verb+--interactive+ - Ручной ввод значений
\item \verb+--is-treat_to_king+- Автоматическая работа, через ввод параметров запуска
\end{enumerate}
...
Модульное тестирование реализовано при помощи фреймворка QtTest.
Так же код проверялся с помощью Cppcheck. Программа Cppcheck указала на ошибки в функциях scanf и printf стандартной библиотеки. Данные ошибки были исправлены, когда подключили заголовочный файл \verb+<stdio.h>+.
\subsection{Тестовый план и результаты тестирования}
...
\begin{enumerate}
\item \verb+--interactive+ - Ручной ввод значений
\item \verb+--is-max_composite_number+- Автоматическая работа, через ввод параметров запуска
\end{enumerate}
...
Так же код проверялся с помощью Cppcheck. Программа Cppcheck указала на ошибки в функциях fscanf и fprintf стандартной библиотеки, на дублированный код, на strcmp, которую лучше не использовать. Данные ошибки были исправлены, когда подключили заголовочный файл \verb+<stdio.h>+ и исправили метод \verb+findings_max_composite_number+, удаляя дублированные части кода. Warning на strcmp исправлению не подлежал, так как именно его более эффективно использовать для данной программы.
\subsection{Тестовый план и результаты тестирования}
...
\subsection{Теоритические сведения}
Класс Stack находится в подпроекте \verb+stack+. В заголовочном файле \verb+stacklib.h+ описан класс, а в исполняемом \verb+stacklib.cpp+ файле реализованы методы класса и конструкторы с деструктором. 
Класс Stackapp находится в подпроекте \verb+stack+. В заголовочном файле \verb+stackapp.h+ описан класс, а в исполняемом \verb+stackapp.cpp+ файле реализованы методы класса и конструкторы с деструктором. Так же в \verb+stackapp.cpp+ "бросается" исключение, на случай если пользователь захочет ввести значение для стека, выходящее за рамки типа int.
В \verb+main.cpp+ реализовано общение с пользователем. 

\subsection{Проектирование}

Метод класса Stack \verb+push+ записывает элементы в поле класса \verb+stackPtr+, метод \verb+printStack+ выводит число в обратном порядке.\\
...
\lstinputlisting[]
{../sources/subproject/stack/stacklib.cpp}
\lstinputlisting[]
{../sources/subproject/stack/stacklib.h}
\lstinputlisting[]
{../sources/subproject/stack/stackapp.h}
\lstinputlisting[]
{../sources/subproject/stack/stackapp.cpp}
